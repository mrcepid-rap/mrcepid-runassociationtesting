...
def ingest_data(association_tarball: str, phenofile: str, inclusion_list: str, exclusion_list: str) -> dict:
...
def process_covariates(sex: int, genetics_samples: set) -> str:
    ...
    for indv in pheno_reader:
        ...
        elif indv[pheno_name].lower() != "na" and indv[pheno_name].lower() != "nan":
    ...
    write_fields = write_fields + ["age", "sex", "batch", "wes_batch"]
...
def bolt(tarball_prefix: str, pheno_name: str) -> None:
    ...
    with open('poss_chromosomes.txt', 'w') as poss_chromosomes:
        for chromosome in CHROMOSOMES:
            if exists(tarball_prefix + "." + chromosome + ".BOLT.bgen"):
                ...
                cmd = "plink2 --bgen /test/" + tarball_prefix + "." + chromosome + ".BOLT.bgen 'ref-last' " \
    ...
    with open('poss_chromosomes.txt', 'w') as poss_chromosomes:
        for chromosome in CHROMOSOMES:
            if exists(tarball_prefix + "." + chromosome + ".BOLT.bgen"):
                ...
                cmd = "plink2 --bgen /test/" + tarball_prefix + "." + chromosome + ".BOLT.markers.bgen 'ref-last' " \
                         "--sample /test/" + tarball_prefix + "." + chromosome + ".BOLT.markers.sample " \
                         "--export bgen-1.2 'bits='8 " \
    ...
    cmd = "bolt " + \
          ...
          "--lmmInfOnly " \
          "--numThreads=32 " \
          "--statsFile=/test/" + tarball_prefix + ".markers.stats.gz " \
          "--verboseStats " \
          "--bgenSampleFileList=/test/poss_chromosomes.txt " \
          "--statsFileBgenSnps=/test/" + tarball_prefix + ".markers.bgen.stats.gz"
    run_cmd(cmd, True)
...
def saige_step_one(tarball_prefix: str, pheno_name: str, is_binary: bool, sex: int) -> None:
    ...
    cmd = "step1_fitNULLGLMM.R " \
          ...
          "--outputPrefix=/test/" + tarball_prefix + ".SAIGE_OUT " \
          "--outputPrefix_varRatio=/test/" + tarball_prefix + ".SAIGE_OUT_cate " \
...
def saige_step_two(tarball_prefix: str, chromosome: str) -> str:

    cmd = "bcftools view --threads 1 -S /test/SAMPLES_Include.txt -Oz -o /test/saige_input." + chromosome + ".vcf.gz /test/" + tarball_prefix + "." + chromosome + ".SAIGE.vcf.gz"
    run_cmd(cmd, True)
    ...
    cmd = "step2_SPAtests.R " \
          ...
          "--vcfField=GT " \
          "--GMMATmodelFile=/test/" + tarball_prefix + ".SAIGE_OUT.rda " \
          "--varianceRatioFile=/test/" + tarball_prefix + ".SAIGE_OUT_cate.varianceRatio.txt " \
          "--LOCO=FALSE " \
...
def staar(tarball_prefix: str, pheno_name: str, is_binary: bool, chromosome: str) -> str:
    ...
    cmd = "Rscript /prog/runSTAAR.R " \
          ...
          "/test/" + tarball_prefix + "." + chromosome + ".REGENIE.annotation " \
...
def linear_model(tarball_prefix, pheno_name, is_binary, chromosome):
    ...
    genes_to_include = open(tarball_prefix + '.' + chromosome + '.REGENIE.inclusion', 'r')
    genes = []
    ...
    cmd = "plink2 --threads 1 --bgen /test/" + tarball_prefix + "." + chromosome + ".BOLT.bgen 'ref-last' --export bcf --out /test/lm." + chromosome
    run_cmd(cmd, True)
    ...
    for gene in genes:
        ...
        if n_car <= 2:
            ...
            sm_results = sm.GLM.from_formula(pheno_name + ' ~ has_var + sex + age + batch + wes_batch + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10', data=pheno_covars, family=family).fit()
    ...
    # Write results:
    lm_stats_file = open(tarball_prefix + "." + chromosome + ".lm_stats.tsv", 'w')
...
def main(association_tarball,
    ...
    injested_data_info = ingest_data(association_tarball, phenofile, inclusion_list, exclusion_list)
    ...
    pheno_name = process_covariates(sex, genetics_samples)
    ...
    if run_bolt:
        ...
        output_files.append(tarball_prefix + ".markers.stats.gz")
        output_files.append(tarball_prefix + ".markers.bgen.stats.gz")
    ...
    if run_saige:
        ...
        saige_step_one(tarball_prefix, pheno_name, is_binary, sex)

        # Run step two WITH parallelisation by chromosome
        print("Running SAIGE step 2...")
        future_pool = []
        for chromosome in CHROMOSOMES:
            if exists(tarball_prefix + "." + chromosome + ".SAIGE.vcf.gz"):
        ...
        for future in futures.as_completed(future_pool):
            try:
                ...
                output_files.append(tarball_prefix + ".SAIGE_OUT.rda")
                output_files.append(tarball_prefix + ".SAIGE_OUT_cate.varianceRatio.txt")
    ...
    if run_staar:
        ...
        for future in futures.as_completed(future_pool):
            try:
                finished_chromosome = future.result()
                output_files.append(tarball_prefix + "." + finished_chromosome + ".STAAR_results.tsv")
            except Exception as err:
                print("A thread failed...")
                print(Exception, err)
    if run_linear_model:
        ...
        for future in futures.as_completed(future_pool):
            try:
                finished_chromosome = future.result()
                output_files.append(tarball_prefix + "." + finished_chromosome + ".lm_stats.tsv")
            except Exception as err:
                print("A thread failed...")
                print(Exception, err)
