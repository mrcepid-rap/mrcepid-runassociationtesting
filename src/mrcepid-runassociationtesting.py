...
def ingest_data(association_tarballs: str, phenofile: str, covarfile: str, inclusion_list: str, exclusion_list: str, run_bolt: bool) -> dict:
...
def process_bolt_outputs(association_pack: dict) -> None:
    ...
    with open(association_pack['output_prefix'] + '.genes.BOLT.stats.tsv', 'w') as gene_out:
        ...
        bolt_table_gene.to_csv(path_or_buf=gene_out, index = False, sep="\t")
        gene_out.close()
    ...
    with open(association_pack['output_prefix'] + '.markers.BOLT.stats.tsv', 'w') as marker_out:
        ...
        bolt_table_marker.to_csv(path_or_buf=marker_out, index = False, sep="\t")
        gene_out.close()
...
def process_staar_outputs(completed_staar_files: list, association_pack: dict) -> None:
    ...
    with open(association_pack['output_prefix'] + '.genes.STAAR.stats.tsv', 'w') as gene_out:
        ...
        staar_table.to_csv(path_or_buf=gene_out, index = False, sep="\t")
        gene_out.close()
        ...
        for chromosome in CHROMOSOMES:
            ...
            if exists(tarball_prefix + "." + chromosome + ".BOLT.bgen"):
                ...
                if genes_obtained == False:
                    with open(tarball_prefix + '.' + chromosome + '.SAIGE.groupFile.txt', 'r') as genes_to_include:
                        for gene in genes_to_include:
                            gene = gene.rstrip().split("\t")[0]
...
def main(association_tarballs,
    ...
    injested_data_info = ingest_data(association_tarballs, phenofile, covarfile, inclusion_list, exclusion_list, run_bolt)
    ...
    if run_linear_model:
        ...
        lm_stats_writer = csv.DictWriter(lm_stats_file,
                                         delimiter = "\t",
                                         fieldnames=['SNP','pheno_name','p_val_init','n_car','n_model','p_val_full','effect','std_err'])
