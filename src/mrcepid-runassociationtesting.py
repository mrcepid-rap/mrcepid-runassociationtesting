...
def select_individuals(inclusion_list: str, exclusion_list: str) -> set:

    # Get a list of individuals that we ARE going to use
    include_samples = set()
    if inclusion_list is not None:
        inclusion_file = open(inclusion_list, 'r')
    ...
    if exclusion_list is not None:
        exclude_file = open(exclusion_list, 'r')
    ...
    for line in genetics_fam_file:
        ...
        fields = line.split()
        eid = fields[0]
        if inclusion_list is None and exclusion_list is None:
            genetics_samples.add(eid)
        elif inclusion_list is None:
...
def bolt(tarball_prefix: str, pheno_name: str) -> None:
    ...
    for chromosome in CHROMOSOMES:
        if exists(tarball_prefix + "." + chromosome + ".BOLT.bgen"):
            poss_chromosomes.append(chromosome)
            ...
            run_cmd(cmd, True)
    poss_chromosomes_string = ','.join(poss_chromosomes)
    ...
    cmd = "bolt " + \
          ...
          "--verboseStats " \
          "--bgenFile=/test/bolt_input.{" + poss_chromosomes_string + "}.bgen " \
          "--sampleFile=/test/bolt_input.1.sample " \
...
def main(association_tarball,
    ...
    genetics_samples = select_individuals(inclusion_list, exclusion_list)
