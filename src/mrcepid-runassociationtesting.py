...
def ingest_data(association_tarballs: str, phenofile: str, covarfile: str, inclusion_list: str, exclusion_list: str) -> dict:
...
def process_bolt_file(tarball_prefix: str, chromosome: str, run_marker_tests: bool) -> None:
    ...
    # And then the markers if requested...
    if run_marker_tests:
        cmd = "plink2 --threads 2 --bgen /test/" + tarball_prefix + "." + chromosome + ".BOLT.markers.bgen 'ref-last' " \
                "--sample /test/" + tarball_prefix + "." + chromosome + ".BOLT.markers.sample " \
                "--export bgen-1.2 'bits='8 " \
                "--out /test/" + tarball_prefix + "." + chromosome + ".markers " \
                "--keep-fam /test/SAMPLES_Include.txt"
        run_cmd(cmd, True)
...
def bolt(association_pack: dict) -> None:
    ...
    available_workers = math.floor((association_pack['threads'] - 1) / 2)
            ...
            for tarball_prefix in association_pack['tarball_prefixes']:
                if exists(tarball_prefix + "." + chromosome + ".BOLT.bgen"):
                    ...
                    if association_pack['run_marker_tests']:
                        poss_chromosomes.write("/test/%s /test/%s\n" % (tarball_prefix + "." + chromosome + ".markers.bgen", tarball_prefix + "." + chromosome + ".markers.sample"))
    ...
    cmd = "bolt " + \
          ...
          "--statsFile=/test/" + association_pack['pheno_name'] + ".stats.gz " \
          "--verboseStats " \
          "--bgenSampleFileList=/test/poss_chromosomes.txt " \
          "--statsFileBgenSnps=/test/" + association_pack['pheno_name'] + ".bgen.stats.gz"
    ...
    if len(association_pack['cat_covars']) > 0:
        for covar in association_pack['cat_covars']:
            cmd += " --covarCol=" + covar + " "
    run_cmd(cmd, True, association_pack['pheno_name'] + ".BOLT.log")
...
def linear_model_setup(association_pack: dict) -> dict:
    ...
    genes = set()
    genes_obtained = False
    for tarball_prefix in association_pack['tarball_prefixes']:
        # For each tarball prefix, we want to make ONE table for efficient querying of variants
        ...
        for chromosome in CHROMOSOMES:
            ...
            if exists(tarball_prefix + "." + chromosome + ".BOLT.bgen"):
                ...
                if genes_obtained == False:
                    with open(tarball_prefix + '.' + chromosome + '.REGENIE.inclusion', 'r') as genes_to_include:
                ...
                # I don't think this is true anymore (as of 450k), but safe to include for legacy purposes since splitting by a
                # nonexistant character does nothing...
        ...
        linear_model_pack['genotypes'][tarball_prefix] = genetic_data
        genes_obtained = True
        print("Finished loading tarball prefix: " + tarball_prefix)

    # Make sure we set genes...
    linear_model_pack['genes'] = genes

    return linear_model_pack

# Run rare variant association testing using GLMs
def linear_model(linear_model_pack: dict, gene: str, mask_name: str) -> dict:

    # Now successively iterate through each gene and run our model:
    # I think this is straight-forward?
    try:
        genotypes = linear_model_pack['genotypes'][mask_name]
        indv_w_var = genotypes.loc[genotypes['gene'] == gene]['eid'].to_list()
        if len(indv_w_var) == 0:
            gene_dict = {'p_val': 'NA',
                         'effect': 'NA',
                         'std_err': 'NA',
...
def main(association_tarballs,
    ...
    if run_bolt:
        ...
        output_files.append(association_pack['pheno_name'] + ".stats.gz")
        output_files.append(association_pack['pheno_name'] + ".bgen.stats.gz")
        output_files.append(association_pack['pheno_name'] + ".BOLT.log")
    ...
    if run_staar:
        ...
        # Here we are compiling a file of each tsv from completed_staar_files:
        with open(association_pack['pheno_name'] + ".STAAR_results.tsv", 'w') as staar_output:
        ...
        output_files.append(association_pack['pheno_name'] + ".STAAR_results.tsv")
        output_files.append(association_pack['pheno_name'] + ".STAAR_null.rds")
    if run_linear_model:
        ...
        # 'genotypes': A dictionary storing "genotypes" for all masks provided to this tool
        linear_model_pack = linear_model_setup(association_pack)
        total_models = len(linear_model_pack['genes']) * len(linear_model_pack['genotypes'])
        ...
        # Write results:
        lm_stats_file = open(association_pack['pheno_name'] + ".lm_stats.tsv", 'w')
        lm_stats_writer = csv.DictWriter(lm_stats_file,
                                         delimiter = "\t",
                                         fieldnames=['SNP','pheno_name','p_val','effect','std_err','n_car','n_model'])
        ...
        output_files.append(association_pack['pheno_name'] + ".lm_stats.tsv")
        lm_stats_file.close()
