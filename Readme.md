...
1. A tab-delimited, gzipped file named like `<PHENO_NAME>.bgen.stats.gz` (where `<PHENO_NAME>` is derived from the header 
   of the provided pheno_file) with the header:

`
SNP     CHR     BP      GENPOS  ALLELE1 ALLELE0 A1FREQ  INFO    CHISQ_LINREG    P_LINREG        BETA    SE      CHISQ_BOLT_LMM_INF      P_BOLT_LMM_INF
`

All columns are identical to standard BOLT-LMM [output](https://alkesgroup.broadinstitute.org/BOLT-LMM/BOLT-LMM_manual.html#x1-480008.1).
SNP-ID has one of two styles:
       
* For gene-based tests, the name will be of the form "ENSTID-MASK-MAF_CUTOFF". Note the '-' delimiter between the 
  three pieces of information. An example ID: "ENST00000641515-MISS-MAF_01". The results for this row will be for the gene
  ENST00000641515, for all missense variants with a MAF ≤ 1e-3
* For marker-based tests, the name will be of the form "CHR:POS:REF:ALT"

2. The standard BOLT log file named like `<PHENO_NAME>.BOLT.log` (where `<PHENO_NAME>` is derived from the header
   of the provided pheno_file).
   
3. The genotype-based results from the first step of BOLT named like `<PHENO_NAME>.BOLT.log` (where `<PHENO_NAME>` is 
   derived from the header of the provided pheno_file).
...
We output a single tab-delimited file for all gene/mask/MAF combinations with the following columns (`<PHENO_NAME>.STAAR_results.tsv`):

1. SNP - ID, MASK, and MAF cutoff of the gene being tested like: ENST00000641515-MISS-MAF_01. The results for this row 
   will be for the gene ENST00000641515, for all missense variants with a MAF ≤ 1e-3.
2. n.samps – number of samples run through STAAR
3. pheno – name of the phenotype from the file provided to `pheno_file` 
4. staar.O.p – Overall p. value
5. staar.SKAT.p – SKAT p. value
6. staar.burden.p – Burden p. value
7. staar.ACAT.p ACAT-V p. value
8. n.var – Number of variants considered for this gene
9. cMAC – Cumulative minor allele count of all variants considered for this gene

We also output the null model in R RDS format (`<PHENO_NAME>.STAAR_null.rds`).
...
|input|description             |
|---- |------------------------|
|association_tarballs  | Hash ID(s) of the output from [mrcepid-mergecollapsevariants](https://github.com/mrcepid-rap/mrcepid-mergecollapsevariants) that you wish to use for rare variant burden testing. See below for more information. |
...
|run_marker_tests| run per-marker tests (only applies to BOLT and SAIGE)? **[FALSE]**|
...
An example file that includes 14 variant masks (7 variant types at two MAF cutoffs) is available at `collapsed_variants/variant_mask_list.txt (file-G7qpz0QJJv8jJ274J3pqzxzB)`
...
to an integer. Samples with **ANY** NaN/NA/blank fields will be skipped by the parser. The user must also provide a comma 
seperated list of covariates to include. Using the example above, all covariates will be included with the following command line:
